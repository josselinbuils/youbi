...
import { EventStatus, WorkerEvent } from './WorkerEvent';
...
class DecodingWorker {
  ...
  private get(event: string, path: string): void {
    ...
    this.asset.get(event, (res: any) => this.success(res));
    this.asset.on('error', (error: Error) => this.error(error.message));
  }
  ...
  init(): void {
    ...
    process.on('message', (command: Command) => {
      ...
      if (['get', 'hasDecoder', 'isActive', 'start', 'stop'].includes(name)) {
        this[name as keyof DecodingWorker].apply(this, args);
      } else {
  ...
  private start(path: string): void {
    ...
    this.asset.on('decodeStart', () => {
      this.asset.decoder.on('data', (typedArray: any) => {
    ...
    this.asset.on('buffer', (progress: number) => {
